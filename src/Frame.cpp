...
namespace ORB_SLAM2
{
    ...
    /******************************VI SLAM*************************/
    // 利用IMU数据计算两帧之间的预积分，保存在IMUPreInt
    void Frame::ComputeIMUPreIntSinceLastFrame(const Frame *pLastF, IMUPreintegrator &IMUPreInt) const
    {
        ...
        // 上一帧到第一个IMU数据的预积分项
        {
            ...
            if (dt < 0)
            {
                cerr << std::fixed << std::setprecision(3) << "dt = " << dt << ", this frame vs last imu time:"
                     << pLastF->mTimeStamp << " vs " << imu._t << endl;
        ...
        for (size_t i = 0; i < vIMUSInceLastFrame.size(); i++)
        {
            ...
            if (dt <= 0)
            {
                cerr << std::fixed << std::setprecision(3) << "dt = " << dt << ", this vs next time: " << imu._t << "vs"
                     << nextt << endl;
                std::cerr.unsetf(std::ios::showbase);
            }
        }
    ...
    Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor *extractorLeft,
            ...
            : mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft), mpORBextractorRight(extractorRight),
              mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mb(0), mThDepth(thDepth),
    ...
    void Frame::ComputeStereoMatches()
    {
        ...
        for (int iL = 0; iL < N; iL++)
        {
            ...
            if (bestDist < ORBmatcher::TH_HIGH)
            {
                ...
                IL = IL -
                     IL.at<float>(w, w) * cv::Mat::ones(IL.rows, IL.cols, CV_32F);//窗口中的每个元素减去正中心的那个元素，简单归一化，减小光照强度影响
