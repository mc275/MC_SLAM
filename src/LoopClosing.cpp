/* 输入：LocalMapping中的局部地图的当前关键帧，在队列mlpLoopKeyFrmaeQueue中。
*  主函数：Run()。
*  输出：Essential Graph（闭环连接关系，Spanning tree,好的共视关系）。
*/
...
namespace ORB_SLAM2
{
    ...
    bool LoopClosing::DetectLoop()
    {
        ...
        mvpEnoughConsistentCandidates.clear();  // 最终筛选的一组闭环帧。

        // ConsistentGroup 类型是pari<set<KeyFrame *>, int> ,第一个表示一致组的关键帧，第二个时每个一致组的序号。
    ...
    bool LoopClosing::ComputeSim3()
    {
        ...
        while(!bMatch && nCandidates>0)
        {
            for(int i=0; i<nInitialCandidates; i++)
            {
                ...
                // 最多迭代5次，返回的Scm是候选帧pKF到当前帧mpCurrentKF的Sim3位姿。
        ...
        // 步骤6 取出闭环关键帧mpMatchedKF的相连关键帧和点云MP。
    ...
    void LoopClosing::CorrectLoop()
    {
        ...
        // 步骤5 更新当前关键的共视关系，MapPoints融合会得到新的连接。
