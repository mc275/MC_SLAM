...
namespace ORB_SLAM2
{
    ...
    //
    void Optimizer::LocalBAPRVIDP(KeyFrame *pCurKF, const std::list<KeyFrame *> &lLocalKeyFrames, bool *pbStopFlag,
                                  Map *pMap, cv::Mat &gw, LocalMapping *pLM)
    {
             ...
             lit != lend; lit++)
        {
            ...
            // Bias 边
            {
                ...
                ebias->setVertex(0,
                                 dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(3 * pKF0->mnId + 2)));
                ebias->setVertex(1,
                                 dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(3 * pKF1->mnId + 2)));
                 ...
                 mit != mend; mit++)
            {
                ...
                if (!pKFi->isBad())
                {
                    ...
                    if (pKFi->mvuRight[mit->second] < 0)
                    {
                        ...
                        e->setVertex(1,
                                     dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(3 * pRefKF->mnId)));
                        e->setVertex(2,
                                     dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(3 * pKFi->mnId)));
                        e->setVertex(3, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                                ExtrinsicVertexId)));
    ...
    // 当前帧和上一帧求解位姿，VI SLAM初始化或地图者更新未完成时使用。
    int
    ...
    // 当前帧和上一关键帧求解位姿，VI SLAM初始化或地图者更新完成时使用。
    int
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                if (pMP)
                {
                    ...
                    if (pFrame->mvuRight[i] < 0)
                    {
                        ...
                        e->setVertex(0,
                                     dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));     // 设置顶点。
                        e->setMeasurement(
                                obs);                                                                 // 设置测量值(u,v)。
                        const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                        e->setInformation(Eigen::Matrix2d::Identity() * invSigma2);
    ...
    **/
                                           ...
                                           LoopClosing *pLC)
    {
             ...
             mit != mend; mit++)
        {
                 ...
                 sit != send; sit++)
            {
                ...
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *> (optimizer.vertex(
                        nIDj)));    // 添加边的顶点，sp中的帧。
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *> (optimizer.vertex(
                        nIDi)));    // 添加边的顶点，sp树的父节点帧。
                e->setMeasurement(
                        Sji);                                                                     // 边的测量值，两顶点间的相对位姿变换。
        ...
        for (size_t i = 0, iend = vpKFs.size(); i < iend; i++)            // 遍历地图中的所有关键帧。
        {
            ...
            for (set<KeyFrame *>::const_iterator sit = sLoopEdges.begin(), send = sLoopEdges.end(); sit != send; sit++)
            {
                ...
                if (pLKF->mnId < pKF->mnId)
                {
                    ...
                    el->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *> (optimizer.vertex(
                            pLKF->mnId)));     // 地图中的某帧关键帧的闭环帧。
                    el->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *> (optimizer.vertex(
                            nIDi)));           // 地图中的某帧关键帧。
                ...
                if (pKFn && pKFn != pParentKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn))
                {
                    if (!pKFn->isBad() && pKFn->mnId < pKF->mnId)
                    {
                        if (sInsertedEdges.count(
                                make_pair(min(pKF->mnId, pKFn->mnId), max(pKF->mnId, pKFn->mnId)))) // 在步骤3中已经添加过。
                            continue;
        ...
        for (size_t i = 0, iend = vpMPs.size(); i < iend; i++)
        {
            ...
            Eigen::Matrix<double, 3, 1> eigCorrectedP3Dw = correctedSwr.map(
                    Srw.map(eigP3Dw));  // 3D坐标 从w->r, r->w，假设3D点在相机坐标下的坐标不变。
